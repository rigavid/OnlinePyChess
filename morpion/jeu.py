...
class morpion:
    ...
    def image(self) -> image:
        ...
        img = new_img(background=COL.green, name=self.name)
        X, Y = ([p1[i]+(p4[i]-p1[i])/3*n for n in range(3)] + [p4[i]] for i in (0, 1))
        for n, x in enumerate(X): img.line([x, p1[1]], [x, p4[1]], COL.black, 10 if n in range(0, 10, 3) else 3, 2)
        for n, y in enumerate(Y): img.line([p1[0], y], [p4[0], y], COL.black, 10 if n in range(0, 10, 3) else 3, 2)
        img.write(f"A {"X" if self.trait else "O"} de\njouer", ct_sg([0, 0], p3), COL.blue if self.trait else COL.red, 3, 2, cv2.FONT_HERSHEY_SIMPLEX, 2)
        for s, c in [[f"X: {self.scores[0]}\n", COL.blue], [f"\nO: {self.scores[1]}", COL.red]]: img.write(s, ct_sg(p2, screen), c, 3, 2, cv2.FONT_HERSHEY_SIMPLEX, 2)
        for x in range(3):
            for y in range(3):
                match self.matrix[x, y]:
                    case 1:
                        offset = dist([X[x],Y[y]],[X[x+1],Y[y]])*0.1
                        for pt1, pt2 in [[[X[x]+offset,Y[y]+offset], [X[x+1]-offset,Y[y+1]-offset]], [[X[x+1]-offset,Y[y]+offset], [X[x]+offset,Y[y+1]-offset]]]: img.line(pt1, pt2, COL.blue, 10, 2)
                    case 2: img.circle(ct_sg([X[x],Y[y]],[X[x+1],Y[y+1]]),dist([X[x],Y[y]],[X[x+1],Y[y]])*0.4, COL.red, 10, 2)
        return img
